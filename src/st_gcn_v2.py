...
MODEL_SAVE_PATH = ROOT_DIR / "best_stgcn_v2.pth"
CHECKPOINT_PATH = ROOT_DIR / "checkpoint_v2.pth"
...
def train(features_dir=None, num_epochs=NUM_EPOCHS,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for joint, bone, y in tqdm(train_loader, desc=f"E{epoch+1:03d}/{num_epochs} train", leave=False):
            ...
            joint_m, bone_m, ya, yb, lam = mixup(joint, bone, y)
            optimizer.zero_grad(set_to_none=True)
            with torch.amp.autocast(device_type=device.type, enabled=use_amp):
                out  = model(joint_m, bone_m)
