"""
ST-GCN v4 — three-stream extension of v2 (joint + bone + motion).

Stays within the canonical multi-stream ST-GCN family (Shi et al. 2019).
Each stream is the same SingleStream architecture as v2, but fed a
specialized 4-channel feature: positions / bone vectors / velocities,
each with the visibility channel. Logits are averaged at fusion time.

Adds two free-win training/eval recipes (both toggleable, default ON):
  * USE_EMA: keeps an exponential moving average of model weights and
    evaluates on those weights as well. Standard 0.999 decay.
  * USE_TTA: at validation time, runs both the original and a
    horizontally-flipped copy of each batch, averages softmax probs.

All other training-recipe choices (DROPOUT=0.5, MIXUP_ALPHA=0.3, attention
on blocks [8,9], clip-level augmentation, plain shuffled DataLoader) are
inherited from v2 because the v3 ablation showed they were load-bearing.
"""
import copy
import glob
import os
...
MODEL_SAVE_PATH = ROOT_DIR / "best_stgcn_v4.pth"
CHECKPOINT_PATH = ROOT_DIR / "checkpoint_v4.pth"
...
GRAD_CLIP     = 1.0
MAX_FRAMES    = 60
IN_CHANNELS   = 4   # x/y/z/vis OR vx/vy/vz/vis OR bx/by/bz/vis (z=0 for 2D)
DROPOUT       = 0.5
DROP_PATH_MAX = 0.3
MIXUP_ALPHA   = 0.3
