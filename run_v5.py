...
def main():
    ...
    ap.add_argument("--restart", action="store_true",
                    help="Wipe existing checkpoint/best files and start fresh (default: resume if checkpoint exists)")
    args = ap.parse_args()

    suffix = "_combined" if args.combined_train else ""
    ckpt = ROOT / f"checkpoint_v5{suffix}.pth"
    best = ROOT / f"best_stgcn_v5{suffix}.pth"
    log  = ROOT / f"compare_v5{suffix}.log"
    if args.restart:
        for p in (ckpt, best):
            if p.exists(): p.unlink()

    import st_gcn_v5 as v5
    print(f"v5: USE_EMA={v5.USE_EMA}  USE_TTA={v5.USE_TTA}  EMA_DECAY={v5.EMA_DECAY}")
    print(f"v5: NUM_NODES={v5.NUM_NODES}  IN_CHANNELS={v5.IN_CHANNELS}")
    ...
    with open(log, log_mode, encoding="utf-8", buffering=1) as f:
        ...
        try:
            v5.train(num_epochs=args.epochs,
    ...
    rows = parse_log(log)
    if not rows:
        print("No epoch summaries parsed. Check the log.")
        return

    print(f"\n=== v5 4-way ablation (single training run) ===")
